...
useEffect(() => {
  ...
  const onFinish = async (values) => {
    ...
    /*
    const departureTime = values.departure; 
    const arrivalTime = values.arrival;

    const departureMinutes = parseInt(departureTime.split(':')[0], 10) * 60 + parseInt(departureTime.split(':')[1], 10);
    const arrivalMinutes = parseInt(arrivalTime.split(':')[0], 10) * 60 + parseInt(arrivalTime.split(':')[1], 10);

    if (arrivalMinutes <= departureMinutes) {
      message.error('Arrival time must be later than departure time.');
      return; 
    }
    */ 
